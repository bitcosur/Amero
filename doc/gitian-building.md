...
*Setup instructions for a Gitian build of Amero Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Amero
...
- [Building Amero Core](#building-amero-core)
...
Clone the git repositories for Amero Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/ameropay/amero
```
...
in the Amero Core repository under 'Fetch and build inputs' to install sources which require
...
To build Amero Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Amero Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/amero/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/ameropay/amero
...
For example:
```bash
URL=https://github.com/crowning-/amero.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit amero=${COMMIT} --url amero=${URL} ../amero/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit amero=${COMMIT} --url amero=${URL} ../amero/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit amero=${COMMIT} --url amero=${URL} ../amero/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the amero git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../amero/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/amero.git
SIGPATH=/some/root/path/amero-detached-sigs.git

./bin/gbuild --url amero=${BTCPATH},signature=${SIGPATH} ../amero/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/amero-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/amero-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/amero-osx-build.assert
```
...
[amero/gitian.sigs](https://github.com/ameropay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
