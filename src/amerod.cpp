...
bool AppInit(int argc, char* argv[])
{
    ...
    {
        std::string strUsage = _("Amero Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  amerod [options]                     " + _("Start Amero Core Daemon") + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "amero:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in amerod anymore. Use the amero-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect amerod signal handlers
    noui_connect();
