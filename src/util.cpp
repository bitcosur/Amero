...
const char * const BITCOIN_CONF_FILENAME = "amero.conf";
const char * const BITCOIN_PID_FILENAME = "amerod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "amero" is a composite category enabling all Amero-related debug output
            if(ptrCategory->count(string("amero"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AmeroCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AmeroCore
    // Mac: ~/Library/Application Support/AmeroCore
    // Unix: ~/.amerocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AmeroCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AmeroCore";
#else
    // Unix
    return pathRet / ".amerocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty amero.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override amero.conf
